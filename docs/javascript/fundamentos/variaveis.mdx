...
<Admonition type="info" title="Considerações Importantes">
  ...
  - **Escopo de Bloco**:<br />
  Tanto `let` quanto `const` respeitam o escopo de bloco, ou seja, são limitados
  ao `{}` em que foram declarados. Isso previne problemas comuns com o `var`,
  como variáveis que “vazam” para fora de loops ou funções.

  - **Hoisting com let e const**:<br/>
  Apesar de serem içadas, as variáveis declaradas com `let` e `const`
  não podem ser usadas antes de sua declaração.
  
  - **Mutabilidade de const**:<br/>
